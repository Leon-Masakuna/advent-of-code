...
const lines = fs.readFileSync("./data.txt", "utf8").split(os.EOL);
...
for (const move of moves) {
  ...
  moveCrate(move.from, move.to, move.qty);
  moveCrates2(move.from, move.to, move.qty);
}

// get the top crate of all stacks
const topCrates = stacks.map((stack) => stack[stack.length - 1]);
console.log("one-by-one", topCrates.join(""));
